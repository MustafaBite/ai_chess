...
public class Square {
    // 0 is A1, 63 is H8. A1=0, B1=1, ..., H8=63
    ...
    public static String toString(int sq) {
        ...
        char file = (char) ('a' + fileOf(sq));
        char rank = (char) ('1' + rankOf(sq));
        return "" + file + rank;
    }

    public static int fromString(String sqStr) {
        if (sqStr == null || sqStr.length() != 2 || sqStr.equals("-")) return NONE;
        int file = sqStr.charAt(0) - 'a';
        int rank = sqStr.charAt(1) - '1';
        return rank * 8 + file;
    }
}
